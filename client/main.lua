...
function OpenCloakroomMenu()
	...
	if Config.EnableNonFreemodePeds then
		table.insert(elements, {label = _U('swat_wear'), value = 'sheriff_wear_freemode', maleModel = 's_m_y_swat_01'})
		table.insert(elements, {label = _U('lieutenant_wear'), value = 'lieutenant_wear_freemode', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01'})
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		table.insert(elements, {label = _U('Armory_2'), value = 'Armory_2'})
    end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'get_stock' then
			OpenGetStocksMenu()
		elseif data.current.value == 'Armory_2' then
			OpenPharmacyMenu()
		end
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy',
	{
		...
		align    = 'bottom-right',
		elements = {
			{label = _U('pharmacy_take1', _U('bulletproof')), value = 'bulletproof'},
			{label = _U('pharmacy_take2', _U('clip')), value = 'clip'}
		}
	}, function(data, menu)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'revive' then
						TriggerServerEvent('esx_ambulancejob:revive5', GetPlayerServerId(closestPlayer))							
...
function SendToCommunityService(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Community Service Menu', {
		...
		if community_services_count == nil then
			...
			TriggerServerEvent("esx_communityservice:sendToCommunityService5", player, community_services_count)
			menu.close()
		end
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill5', GetPlayerServerId(player), 'society_police', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('esx_billing:sendBill5', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
